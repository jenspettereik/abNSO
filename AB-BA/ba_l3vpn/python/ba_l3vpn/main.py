...
class ServiceCallbacks(Service):
    ...
    def cb_create(self, tctx, root, service, proplist):
        ...
        for endpoint in service.endpoint:
            ...
            ########## INTERFACE POLICY MAPS SECTION ########
            name = "VRF_POLICY-"+endpoint.csr.device+"-"+str(endpoint.id)
            vars.add('name', name)
            vars.add('run_as_rfs', True)
            ...
            template.apply('ba_rpl_qos_rfs-template', vars)
            name = "UL_POLICY-"+endpoint.csr.device+"-"+str(endpoint.id)
            vars.add('name', name)
            ...
            self.log.info('POLICY MAP: ', endpoint.csr.uplink_policy_map)
            self.log.info('INTERFACE NAME: ', endpoint.csr.link.interface_name)
            self.log.info('INTERFACE NUMBER: ', endpoint.csr.link.interface_number)
            template.apply('ba_rpl_qos_rfs-template', vars)
            name = "DL_POLICY-"+endpoint.er.device+"-"+str(endpoint.id)
            vars.add('name', name)
            ...
            self.log.info('POLICY MAP: ', endpoint.er.downlink_policy_map)
            self.log.info('INTERFACE NAME: ', endpoint.er.link.interface_name)
            self.log.info('INTERFACE NUMBER: ', endpoint.er.link.interface_number)
            template.apply('ba_rpl_qos_rfs-template', vars)
